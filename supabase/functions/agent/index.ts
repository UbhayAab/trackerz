...
const FOOD_WORDS = ["aam","almond","almonds","aloo","aloo bhujia","aloo bhujiya","aloo gobi","aloo kathi roll","aloo paratha","aloo parathas","aloo roll","aloo rolls","aloo tikki burger","americano","amrood","anda","anda curry","anda roll","ande","apple","apples","ate","badam","baked chips","banana","bananas","barfi","besan laddu","bhaat","bhaji","bhindi","bhujia","bhujiya","biryani","biscuit","biscuits","bk paneer wrap","black coffee","black forest","black tea","blueberries","blueberry","boiled egg","boiled eggs","boiled rice","boiled soya","boiled soya beans","boiled soybean","boiled soybeans","bread","bread slice","bread slices","breakfast","brownie","brownies","burfi","burger","burrito","burritos","butter","butter chicken","buttermilk","cafe latte","cake","cake slice","cappuccino","caramel popcorn","carrot halwa","chaas","chaat","chai","chana","chana masala","chapathi","chapati","chapatis","chatni","chawal","cheese","cheese popcorn","cheese slice","cheese slices","chhaas","chhole","chia","chia seeds","chicken","chicken 100g","chicken biryani","chicken breast","chicken burger","chicken curry","chicken gravy","chicken kathi roll","chicken patty burger","chicken roll","chicken rolls","chicken wrap","chickpea curry","chips","choc chip cookie","choc chip cookies","choco bar","choco chip cookie","choco chip cookies","chocolate","chocolate bar","chocolate chip cookie","chocolate chip cookies","chole","chutney","coca cola","coca cola zero","cocacola","coconut chutney","coffee","coke","coke zero","cola","cooked soya","cooked soybean","cooked soybeans","cookie","cookies","cottage cheese","cream biscuit","crispy veg whopper","crunchy veg whopper","cucumber","cucumbers","cup cake","cupcake","curd","curry","daal","dahi","dairy milk","dal","dal bowl","dal fry","diet coke","diet pepsi","diet soda","diet soft drink","dinner","donut","donuts","doodh","doodh chai","dosa","dosas","double anda roll","double egg roll","double egg rolls","doughnut","doughnuts","dumpling","dumplings","eaten","egg","egg curry","egg kathi roll","egg masala","egg roll","egg rolls","egg white","egg whites","eggs","espresso","fanta","filter coffee","finger chips","fish","fish curry","flax seeds","food","frankie","french fries","fries","fruit","fruit chaat","fruit juice","fruit salad","fulka","gajar halwa","gelato","granola","greek yoghurt","greek yogurt","green chutney","green salad","green tea","grilled chicken","grilled sandwich","groundnut","guava","gulab jamun","gulab jamuns","gulabjamun","halva","halwa","hung curd","ice cream","ice cream scoop","ice creams","icecream","idli","idlis","idly","instant noodles","instant soup","jalebi","jalebis","jam","jeera rice","juice","kaju barfi","kaju katli","kakdi","kathi roll","kebab","kela","kheer","kheera","khichdi","kidney beans","king fries","knorr soup","kulcha","kulfi","kulfis","laddoo","laddu","ladoo","lamb curry","large french fries","large fries","lassi","latte","lays","lemon tea","lentils","lg fries","limca","lunch","macaroni","maggi","makhan","mango","mango juice","mangoes","marmalade","masala chai","masala dosa","masala dosas","mass gainer shake","matka kulfi","mcchicken","meal","meal maker","med fries","medium french fries","medium fries","milk","milk coffee","milk tea","millet chip","millet chips","mirinda","mixed veg","mixture","momo","momos","moongphali","motichoor laddu","muesli","mutton","mutton curry","naan","nacho","nachos","namkeen","noodles","nutrela","oatmeal","oats","omelet","omelette","onion","onions","orange","orange juice","oranges","pakoda","pakora","palak","paneer","paneer kathi roll","paneer roll","paneer rolls","paneer wrap","pani roll","pani rolls","panner roll","panner rolls","pao bhaji","parantha","paratha","parathas","pasta","pastry","pav bhaji","payasam","peanut butter","peanuts","pepsi","pepsi black","pepsi zero","phulka","phulkas","pizza","pizza slice","pizza slices","plain dosa","poha","poori","popcorn","porridge","potato chips","potato paratha","potato roll","potato rolls","prawns","protein milk shake","protein powder","protein scoop","protein shake","pulao","pumpkin seeds","puri","pyaaz","pyaz","quesadilla","quesadillas","ragi chip","ragi chips","rajma","ramen","ras malai","rasgulla","rasgullas","rasmalai","rasmalais","regular fries","rice","rice bowl","rice pudding","roasted peanuts","rosogolla","roti","rotis","rusk","sabji","sabzi","salad","salad bowl","sambar","sambhar","samosa","samosas","sandwich","santra","scoop of whey","scoop whey","seb","seed mix","seeds","sev","sewai","shake","shawarma","single egg roll","slice of bread","sm fries","small french fries","small fries","smoothie","snack","soda","soft drink","soft serve","softy","sooji halwa","soop sachet","soup","soup packet","soup sachet","soup sachets","south indian coffee","soy chunks","soya","soya bean","soya beans","soya chunk","soya chunks","soyabean","soyabeans","soybean","soybeans","sprite","sprite zero","sprouts","steamed rice","sugar free cola","suji halwa","sundae","sunflower seeds","sweet lassi","tadka dal","tamatar","tea","textured vegetable protein","thakkali sadam","thali","thums up","thums up zero","tikka","toast","toast biscuit","tofu","tomato","tomato bhaat","tomato pulao","tomato rice","tomatoes","toned milk","triple egg roll","triple egg rolls","tvp","upma","uttapam","vada","vada pao","vada pav","veg biryani","veg burger","veg curry","veg kathi roll","veg roll","veg rolls","veg salad","veg sandwich","veg whopper","veg wrap","vegetable biryani","vegetable roll","vegetarian whopper","veggie whopper","wafers","whey","whey powder","whey protein","whey scoop","white rice","white sauce pasta","whites","whole egg","whole eggs","wrap","wraps","yoghurt","yogurt","zero sugar cola"];
...
const FOOD_TABLE: any[] = [
  ...
  { key: "tomato rice", kind: "count", gramsPerUnit: 150, aliases: ["tomato rice", "thakkali sadam", "tomato bhaat", "tomato pulao"], calories: 300, protein_g: 5.5, carbs_g: 47, fat_g: 10 },
  ...
  { key: "egg roll", kind: "count", gramsPerUnit: 180, aliases: ["egg roll", "egg rolls", "single egg roll", "anda roll", "egg kathi roll"], calories: 360, protein_g: 12, carbs_g: 36, fat_g: 18 },
  { key: "double egg roll", kind: "count", gramsPerUnit: 230, aliases: ["double egg roll", "double egg rolls", "double anda roll"], calories: 430, protein_g: 18, carbs_g: 37, fat_g: 23 },
  { key: "triple egg roll", kind: "count", gramsPerUnit: 280, aliases: ["triple egg roll", "triple egg rolls"], calories: 500, protein_g: 24, carbs_g: 37, fat_g: 28 },
  { key: "paneer roll", kind: "count", gramsPerUnit: 220, aliases: ["paneer roll", "paneer rolls", "pani roll", "pani rolls", "panner roll", "panner rolls", "paneer kathi roll"], calories: 450, protein_g: 17, carbs_g: 40, fat_g: 25 },
  { key: "aloo roll", kind: "count", gramsPerUnit: 240, aliases: ["aloo roll", "aloo rolls", "potato roll", "potato rolls", "aloo kathi roll"], calories: 430, protein_g: 9, carbs_g: 57, fat_g: 19 },
  { key: "chicken roll", kind: "count", gramsPerUnit: 220, aliases: ["chicken roll", "chicken rolls", "chicken kathi roll"], calories: 460, protein_g: 24, carbs_g: 38, fat_g: 23 },
  { key: "veg roll", kind: "count", gramsPerUnit: 210, aliases: ["veg roll", "veg rolls", "vegetable roll", "veg kathi roll"], calories: 380, protein_g: 8, carbs_g: 48, fat_g: 17 },
  ...
  { key: "veg whopper", kind: "count", gramsPerUnit: 285, aliases: ["veg whopper", "vegetarian whopper", "veggie whopper", "crispy veg whopper", "crunchy veg whopper"], calories: 682, protein_g: 16, carbs_g: 97, fat_g: 25.4 },
  { key: "paneer wrap", kind: "count", gramsPerUnit: 230, aliases: ["paneer wrap", "bk paneer wrap"], calories: 500, protein_g: 17, carbs_g: 52, fat_g: 25 },
  { key: "wrap", kind: "count", gramsPerUnit: 240, aliases: ["wrap", "wraps", "kathi roll", "veg wrap", "chicken wrap", "frankie"], calories: 460, protein_g: 16, carbs_g: 51, fat_g: 21 },
  { key: "small fries", kind: "count", gramsPerUnit: 72, aliases: ["small fries", "regular fries", "small french fries", "sm fries"], calories: 204, protein_g: 3.6, carbs_g: 27.4, fat_g: 8.9 },
  { key: "fries", kind: "count", gramsPerUnit: 114, aliases: ["medium fries", "med fries", "french fries", "fries", "finger chips", "medium french fries"], calories: 333, protein_g: 5.9, carbs_g: 43.3, fat_g: 14.4 },
  { key: "large fries", kind: "count", gramsPerUnit: 156, aliases: ["large fries", "king fries", "large french fries", "lg fries"], calories: 455, protein_g: 8, carbs_g: 59.3, fat_g: 19.7 },
...
// Regenerated from lib/food-nutrition.mjs by scripts/sync-mirror.mjs.
const FOOD_BRAND_PHRASES = ["burger king","burger singh"];
...
function foodParsePhrase(phrase: string, lineQty: number | null = null): { items: any[]; unknown: string[] } {
  let masked = ` ${phrase} `;
  // Mirror of lib/food-nutrition.mjs: blank out brand names FIRST, so a
  // restaurant cannot be eaten. "Burger King" contains the alias `burger`, and a
  // matched alias is a confident answer rather than a gap - this is the copy
  // that decides, because the server's totals override the model's.
  for (const brand of FOOD_BRAND_PHRASES) {
    const rx = new RegExp(`(?<![a-z])${brand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![a-z])`, "g");
    masked = masked.replace(rx, (m: string) => "\0".repeat(m.length));
  }
  const found: any[] = [];
