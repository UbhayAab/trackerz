...
// A permanent GYM override from user_plans (payload). A pasted weekly split
// ("here's my new gym plan from ChatGPT") stores either one workout or a
// per-weekday {days:{Monday:{...}}} map; planForDate resolves it before the
// standing A/B/cardio cycle. Deleting the row (undo) restores the cycle.
...
// Pick a weekday's spec out of a {days:{Mon:{...}}} / {days:{Monday:{...}}} map
// (case-insensitive on the day key). Returns null when the payload has no days
// map or nothing for that weekday.
function daySpecFromMap(payload, wd) {
  if (!payload?.days || typeof payload.days !== "object") return null;
  const short = ["", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][wd];
  const full = WEEKDAY_NAMES[wd];
  if (payload.days[short]) return payload.days[short];
  if (payload.days[full]) return payload.days[full];
  const want = new Set([short.toLowerCase(), full.toLowerCase()]);
  for (const [k, v] of Object.entries(payload.days)) {
    if (want.has(String(k).trim().toLowerCase())) return v;
  }
  return null;
}

// Build a workout object from a gym override payload (items list, or a
...
function gymWorkoutFromPayload(payload, wd) {
  ...
  if (payload.days && typeof payload.days === "object") {
    spec = daySpecFromMap(payload, wd);
  }
...
export function planForDate(date = new Date()) {
  ...
  // Diet: a permanent override (single-day {meals,targets} OR a weekly
  // {days:{Monday:{meals,targets}}} map) and/or this date's rows fold onto the
  // scaffold. A full payload replaces the day; a delta ({op}) merges onto the
  // current base (the permanent override's day if there is one, else the
  // standing scaffold).
  const permDietDay = _dietOverride
    ? (daySpecFromMap(_dietOverride, wd) || (Array.isArray(_dietOverride.meals) ? _dietOverride : null))
    : null;
  const datedDietRows = normRows(_dietDated.get(key));
  let dietPayload = null;
  if (permDietDay || datedDietRows.length) {
    const base = permDietDay || scaffoldDietPayload(date);
    dietPayload = datedDietRows.length ? foldPlanPayloads("diet", base, datedDietRows) : permDietDay;
  }
  const meals = (dietPayload && overrideMeals(dietPayload)) || mealsFor(dietType);

  // Gym: a permanent override (one workout or a weekly days map) replaces the
  // standing A/B/cardio cycle; this date's rows then fold onto that base.
  const permWorkout = gymWorkoutFromPayload(_gymOverride, wd);
  const permGymPayload = permWorkout
    ? { name: permWorkout.name, kind: permWorkout.kind, duration_min: permWorkout.duration_min, items: [...permWorkout.items], rules: permWorkout.rules }
    : null;
  const datedGymRows = normRows(_gymDated.get(key));
  let datedWorkout = null;
  if (datedGymRows.length) {
    const gymPayload = foldPlanPayloads("gym", permGymPayload || scaffoldGymPayload(date), datedGymRows);
  ...
  return {
    ...
    workout: datedWorkout || permWorkout || WORKOUTS[WORKOUT_BY_WEEKDAY[wd]],
    meals,
    customDiet: Boolean(dietPayload),
    customWorkout: Boolean(datedWorkout || permWorkout),
