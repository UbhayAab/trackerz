...
import { protocolWorkouts, PROTOCOL_BY_WEEKDAY } from "./home-protocol.mjs";
...
// The training program is the Home Protocol (lib/home-protocol.mjs): six days
// of full-body work under a 25 kg dumbbell ceiling, then a rest day. It replaced
// the gym-floor scaffold that used to live here inline - Workout A / Workout B /
// a "forgiven cardio" day - which prescribed a treadmill, a leg press and a lat
// pulldown that the room training now happens in does not contain.
//
// WORKOUTS keeps its old shape (id/name/kind/items/rules) so plan-merge, the
// morning brief and the diet page need no change; each entry also carries the
// structured `exercises` array the gym panel logs against.
export const WORKOUTS = protocolWorkouts();

// Mon-Sat are A-F, Sunday is the rest day. One calendar day, one rotation day.
export const WORKOUT_BY_WEEKDAY = PROTOCOL_BY_WEEKDAY;
...
// DELETED 2026-08-06: prepForTomorrow().
//
// It returned five fixed sentences - soak 50 g dry soybeans, keep 3 whole + 4
// whites, chill 200 g hung curd - keyed off nothing but the weekday. The owner
// stopped eating hung curd and the card went on asking them to chill some, every
// night, which is how a checklist teaches you to stop reading it. Same disease as
// the phantom meal: a JS template rendered where the user expects a measurement.
//
// Prep is now DERIVED from tomorrow's real meals in lib/diet-prep.mjs, and the
// panel renders nothing at all while tomorrow is still this scaffold - advice
// generated from a placeholder is fiction with a checkbox next to it.
//
// The stub is not kept. A deleted function that still exists is one an autocomplete
// will offer back.
