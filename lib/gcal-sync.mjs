...
import { dayKeyInTz, minuteOfDayInTz, hhmm, DEFAULT_TZ } from "./tz.mjs";
...
/**
 * A mirrored third-party event -> a row the calendar can draw.
 *
 * WHY THIS EXISTS. The pull path has always written `calendar_events_raw`, and
 * `fetchMirroredEvents()` has always been able to read it - but nothing rendered
 * it. On 2026-08-08 the whole feature had exactly one consumer, and that consumer
 * was a test. So connecting Google would have imported a year of meetings into a
 * calendar that still showed nothing but reminder rules, which is the same empty
 * screen the user was already complaining about, now with a syncing engine
 * behind it.
 *
 * A mirrored event is a `once` rule with a date and (usually) a time, which is
 * precisely the shape lib/reminders.mjs already draws. `is_event` marks it as
 * NOT the user's own rule: it is read-only, it must never be offered a Pause or
 * a Delete, and a multi-day event is expanded by the caller, not invented here.
 *
 * A recurring remote event is stored as one row per INSTANCE by the pull path
 * (Google's singleEvents expansion), so nothing here needs to expand a rule.
 */
export function mirroredEventRow(ev, { timeZone = DEFAULT_TZ } = {}) {
  if (!ev) return null;
  // A cancelled meeting that still renders is worse than one that never
  // rendered, because the user plans around it.
  if (String(ev.status || "").toLowerCase() === "cancelled") return null;
  if (ev.tombstoned_at) return null;

  let onDate = null;
  let atTime = null;
  if (ev.all_day || (!ev.starts_at && ev.start_date)) {
    onDate = String(ev.start_date || "").slice(0, 10) || null;
  } else if (ev.starts_at) {
    const ms = Date.parse(String(ev.starts_at));
    if (!Number.isFinite(ms)) return null;
    onDate = dayKeyInTz(ms, timeZone);
    atTime = hhmm(minuteOfDayInTz(ms, timeZone));
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(onDate || ""))) return null;

  return {
    id: `gcal:${ev.id || ev.event_id || onDate}`,
    title: String(ev.summary || "(no title)"),
    // Its own glyph. A meeting somebody else owns is not one of the user's
    // reminder kinds, and dressing it as `appointment` would let it be mistaken
    // for a rule they can edit.
    kind: "event",
    freq: "once",
    on_date: onDate,
    at_time: atTime,
    ...
    lead_days: 0,
    is_event: true,
    note: ev.location ? String(ev.location) : null,
...
/** The whole list, unrenderable rows dropped rather than drawn as blanks. */
export function mirroredEventRows(events, opts = {}) {
  const out = [];
  for (const ev of events || []) {
    const row = mirroredEventRow(ev, opts);
    if (row) out.push(row);
  }
  return out;
}
