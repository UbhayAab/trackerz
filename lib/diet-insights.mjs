...
function emptyDay() {
  return { protein: 0, calories: 0, carbs: 0, fat: 0, count: 0, proteinUnknown: 0, caloriesUnknown: 0 };
}

// Is this macro actually MEASURED on this row, or absent?
//
// num() maps null/undefined/"" to 0, which is right for arithmetic and wrong for
// truth: a meal whose protein was never estimated is not a meal with no protein.
// On the owner's real data exactly one row carries this - a 2500 kcal restaurant
// dinner logged 2026-07-23 IST with protein_g, carbs_g and fat_g all NULL - and
// it made the next day's "45.4 g protein" a sum of two rows out of three,
// reported as if it were the whole day. Counting the absences is what lets a
// caller say "at least 45.4 g" instead of "45.4 g".
function known(v) {
...
function groupByDay(foodLogs, tz) {
  ...
  for (const row of foodLogs || []) {
    ...
    d.fat += num(row.fat_g);
    d.count += 1;
    if (!known(row.protein_g)) d.proteinUnknown += 1;
    if (!known(row.calories_estimate)) d.caloriesUnknown += 1;
  }
  return byDay;
}
...
export function computeDietInsights(foodLogs, targets, options = {}) {
  ...
  const series = keys.map((key) => {
    ...
    return {
      ...
      // The day's number is a floor, not a total: at least one meal's macro was
      // never estimated, so the real figure can only be higher.
      proteinPartial: !!(d && d.proteinUnknown > 0),
      caloriesPartial: !!(d && d.caloriesUnknown > 0),
      unknownMeals: d ? d.proteinUnknown : 0,
  ...
  //
  // A day holding a meal with unmeasured protein CANNOT support that claim: its
  // known sum is a floor, and the missing meal could have carried it over the
  // target. Counting such a day as a miss is the same error as summing the
  // absent macro to zero, one layer up.
  const partialDays = loggedDays.filter((s) => s.proteinPartial).length;
  const consistentlyShort = !thin
    && partialDays === 0
    && loggedDays.every((s) => s.protein < proteinTarget);
  ...
  return {
    ...
    daysWithData,
    thin,
    // `userSet` is false when NOBODY chose this number - it is the diet
    // scaffold's default. Callers must label it rather than report it back as
    // "your target". Pass `targetUserSet: true` once a budgets row exists.
    target: { protein_g: proteinTarget, calories: calorieTarget, userSet: options.targetUserSet === true },
    protein: {
      today: proteinToday,
      avg: proteinAvg == null ? null : round(proteinAvg, 1),
      target: proteinTarget,
      targetUserSet: options.targetUserSet === true,
      todayGap,
      avgGap,
      consistentlyShort,
      // Today's number is a floor, not a total, when a meal's protein is absent.
      todayPartial: !!todayEntry.proteinPartial,
      todayUnknownMeals: todayEntry.unknownMeals || 0,
      // Logged days in the window whose protein sum is incomplete.
      partialDays,
    },
    calories: {
      today: caloriesToday,
