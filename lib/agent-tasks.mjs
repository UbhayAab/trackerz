...
/**
 * An agent_tasks row shaped as a reminder RULE, so the calendar renders both
 * from one code path.
 *
 * The owner's sentence was "Calendar is where it will be putting down all its
 * scheduled tasks". A task the app scheduled for itself that only exists in a
 * table is not on a calendar - it is invisible, and an invisible autonomous
 * agenda is precisely the thing that makes autonomy feel out of control.
 *
 * Returns null for a task that will never fire again (done / failed / disabled,
 * or a one-shot already in the past), because a calendar showing dates that
 * cannot happen is worse than one that omits them.
 */
export function taskAsCalendarRow(task, today, tz = DEFAULT_TZ) {
  const t = task || {};
  if (t.status !== "scheduled") return null;
  const zone = t.tz || tz || DEFAULT_TZ;
  const ms = Date.parse(String(t.fire_at || ""));
  if (!Number.isFinite(ms)) return null;
  const fireKey = dayKeyInTz(ms, zone);
  const rule = t.recurrence && typeof t.recurrence === "object" ? t.recurrence : null;
  if (!rule && today && fireKey < String(today)) return null;

  // The prompt is an instruction written for the model, not a label for a human.
  // Trim it to a title-length phrase and keep the whole thing as the note, so the
  // agenda line stays readable and nothing is lost.
  const prompt = String(t.prompt || "").trim();
  const title = prompt.length > 60 ? `${prompt.slice(0, 57).replace(/\s+\S*$/, "")}...` : (prompt || "Scheduled check");
  ...
  const base = {
    ...
    note: prompt,
    kind: "agent",
    intent: t.intent || "check",
    is_task: true,
    active: true,
    lead_days: 0,
    timezone: zone,
  };
  if (rule) return { ...base, ...rule, at_time: rule.at_time || hhmmInTz(ms, zone), dtstart: rule.dtstart || fireKey };
  return { ...base, freq: "once", on_date: fireKey, at_time: hhmmInTz(ms, zone) };
}
