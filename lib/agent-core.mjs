...
export function classifyCaptureInput({ text = "", files = [] }) {
  ...
  if (/\b(xls|xlsx|csv|statement|bank|credit card|pdf)\b/.test(fileKinds) || /\bstatement|excel|csv|bank export\b/.test(lower)) {
    return "file_import";
  }

  if (/\b(spent|paid|rs|inr|upi|gpay|phonepe|paytm|debit|credit|refund|zomato|swiggy)\b/.test(lower)) {
    return "money";
  }
...
export function routeModelForCapture({ captureType, risk = "normal" }) {
  ...
  if (captureType === "media_review") {
    return {
      mediaModel: risk === "high" ? "gemini-3.1-pro-preview" : "gemini-3.1-flash-lite",
      brainModel: "deepseek-ai/deepseek-v4-pro",
