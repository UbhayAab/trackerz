...
// CEASING is a denial too. "I should stop eating the aloo bhujia in the evening"
// names a food and is the exact opposite of eating it, but the clause carried no
// skip or miss verb, so nothing denied it and salvage priced 100 g of bhujia into
// the day - twice, 1,140 kcal the owner never ate.
//
// The consumption verb is REQUIRED after the cease verb. Making it optional
// looked more general and immediately swallowed "stopped at the shop and had a
// samosa": the clause splitter does not break on "and", so that whole sentence is
// ONE clause, it mentions a samosa, and a bare "stopped" then denied a real meal.
//
// The cost is that "giving up sugar" is not caught here, only "giving up eating
// sugar". That is the right trade, and worth stating because it will look like an
// omission: this guard SUPPRESSES user data, so a miss costs a row the owner can
// delete, while a false positive costs a meal that silently never existed.
...
// ADMINISTRATIVE MENTIONS. "remind me to renew my gym membership on 20 September"
// names the gym and is paperwork, not a session - salvage read it as a completed
// workout dated 20 September, a training session in the future. Same family as
// the phantom meal: naming a domain is evidence of DOMAIN, never of OCCURRENCE.
//
// Requires the admin noun, so "paid 900 at the gym cafe then trained" is untouched.
...
/** Is this clause about the ADMIN of a domain rather than doing it? */
export function clauseIsAdministrative(clause = "") {
...
export function clauseDeniesEvent(clause = "") {
  ...
  return NEG_NO_EVENT.test(t) || NEG_AUX_VERB.test(t) || NEG_DENIAL_VERB.test(t)
    || NEG_IDIOM.test(t) || NEG_CEASE.test(t) || NEG_ADMIN.test(t);
}
