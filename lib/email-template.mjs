...
//
// Pure (no DOM, no runtime globals, no Supabase) so it is unit-tested; the
// jarvis edge function keeps a byte-identical mirror between the marker
// comments. Note "Deno" in the copy below is the APP name; the edge runtime of
// the same name is unrelated, which is why internal identifiers stayed `jarvis`.
