...
// ---------------------------------------------------------------------------
// Everything below is OUTSIDE the mirror block on purpose: jarvis does not need
// it, and widening the mirror would force a sync of a file this change does not
// otherwise touch.
// ---------------------------------------------------------------------------
...
// "19 October 2002" / "October 19, 2002" / "19th Oct". Returns the parts, never
// a Date - the year is optional and its ABSENCE has to stay distinguishable from
// a guessed one, which is exactly what a Date object cannot express.
function parseSpokenDate(s) {
  const t = String(s || "").toLowerCase();
  const names = Object.keys(MONTH_WORDS).sort((a, b) => b.length - a.length).join("|");
  let m = new RegExp(`\\b(\\d{1,2})(?:st|nd|rd|th)?\\s+(?:of\\s+)?(${names})\\b\\.?(?:[,\\s]+(\\d{4}))?`).exec(t);
  if (m) return { d: Number(m[1]), m: MONTH_WORDS[m[2]], y: m[3] ? Number(m[3]) : null };
  m = new RegExp(`\\b(${names})\\b\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:[,\\s]+(\\d{4}))?`).exec(t);
  if (m) return { d: Number(m[2]), m: MONTH_WORDS[m[1]], y: m[3] ? Number(m[3]) : null };
  return null;
}

// Who the date belongs to, as a title. The possessive right before the noun is
// the whole signal: "my mother's birthday" -> "Mother's birthday", "my birthday"
// -> "My birthday". Anything else gets the bare noun rather than an invented owner.
function ownerTitle(before, noun) {
  const t = String(before || "").toLowerCase().trim().replace(/\s+/g, " ");
  const own = /(?:^|\s)([a-z][a-z'-]*(?:'s|s'))(?:\s+([a-z]+))?\s*$/.exec(t);
  if (own) {
    const who = own[1].replace(/(?:'s|s')$/, "");
    const mod = own[2] ? `${own[2]} ` : "";
    return `${who.charAt(0).toUpperCase()}${who.slice(1)}'s ${mod}${noun}`;
  }
  ...
  const lead = bare ? `${bare[1]} ` : "";
  const s = `${lead}${noun}`;
  return s.charAt(0).toUpperCase() + s.slice(1);
}

// The connector is REQUIRED and is deliberately narrow. With a bare "on" allowed,
// "birthday party on 3 May" - a one-off event - became a yearly birthday rule.
// This layer is a BACKSTOP behind a model that already handles the loose
// phrasings, so precision beats recall: a wrong yearly rule fires every year.
const DATED_FACT_RE =
  /((?:[a-z][a-z'-]*\s+){0,3}?)\b(birthdays?|bday|birth\s*dates?|birth\s*days?|anniversary)\b\s*(?:(?:is|was|falls|comes|will\s+be)\s*(?:on\s+)?|[:\-]\s*)/gi;

/**
 * "my birthday is 19 October 2002" -> the reminder row it should have become.
 *
 * WHY THIS IS DETERMINISTIC AND NOT LEFT TO THE MODEL. Measured against the
 * deployed function on 2026-08-08: "my mother's birthday is 12 March" wrote a
 * correct yearly reminder, and "my birthday is 19 October 2002" wrote NOTHING -
 * because `memory_facts.birthday` already held "19 October 2002" and the brain
 * obeyed the prompt's ALREADY-LOGGED rule. Knowing a date is not the same as
 * having it on the calendar, so a fact in the context must never be able to
 * cancel the calendar entry. A rule the user can state in one breath deserves a
 * path that does not depend on what else is in the context window.
 *
 * The YEAR is kept, in `dtstart`. It is the only place the schema can hold it,
 * it makes the rule no less correct (a yearly rule anchored to the birth date
 * still lands on the same day), and it is what lets the calendar say "turns 24"
 * instead of dropping the fact the user actually said.
 *
 * Returns an array (a sentence can carry two) or [].
 */
function datedFactReminders(text, { today = null } = {}) {
  const src = String(text || "");
  if (!src) return [];
  const out = [];
  const seen = new Set();
  const thisYear = parseKey(today) ? parseKey(today).y : null;
  DATED_FACT_RE.lastIndex = 0;
  let m;
  while ((m = DATED_FACT_RE.exec(src)) != null) {
    // Every spelling collapses to one of two nouns, so "bday" and "birth date"
    // do not become two different titles for the same fact.
    const noun = /anniversary/i.test(m[2]) ? "anniversary" : "birthday";
    const tail = src.slice(m.index + m[0].length, m.index + m[0].length + 40);
    const when = parseSpokenDate(tail);
    if (!when || !when.m || when.d < 1 || when.d > daysInMonth(2004, when.m)) continue;
    const title = ownerTitle(m[1], noun);
    const key = `${title}|${when.m}|${when.d}`;
    if (seen.has(key)) continue;
    seen.add(key);
    const row = {
      title,
      kind: noun === "anniversary" ? "anniversary" : "birthday",
      freq: "yearly",
      month_of_year: when.m,
      day_of_month: when.d,
      lead_days: 2,
    };
    // A year in the future is not a birth year - it is a typo or a different
    // kind of date, and anchoring the series to it would silence the rule until
    // then. Dropped rather than guessed at.
    if (when.y && when.y >= 1900 && (thisYear == null || when.y <= thisYear)) {
      row.dtstart = toKey(when.y, when.m, clampDay(when.y, when.m, when.d));
    }
    out.push(row);
  }
  return out;
...
/** The first one, or null - the shape most callers want. */
function datedFactReminder(text, opts = {}) {
  return datedFactReminders(text, opts)[0] || null;
}

/**
 * Make a birth date the user stated reach the calendar, once, with a title that
 * is a function of what they SAID rather than of how the model phrased it.
 *
 * Two failures, one fix. (1) When `memory_facts.birthday` already held the date,
 * the brain emitted nothing at all and the capture read as lost. (2) Once it did
 * emit, it renamed "my birthday" to "Ubhay's birthday" - a different title for
 * the same fact, so the row could never be recognised as one the user already
 * had, and every mention wrote another 19 October.
 *
 * So: any birthday/anniversary call whose month and day match a date stated in
 * the text takes THAT sentence's title and year, and is flagged `_dated_fact` so
 * the writer can look for the row before inserting a second one. When the model
 * emitted no reminder at all, the stated fact is appended. A named owner stays
 * distinct - "Priya's birthday" is derived from the text too - so nothing merges
 * two different people who happen to share a date.
 *
 * Pure and shape-agnostic: `calls` are `{ name, arguments }` objects.
 */
function normalizeDatedFactCalls(calls, text, { today = null } = {}) {
  const facts = datedFactReminders(text, { today });
  ...
  const list = (calls || []).map((tc) => {
    ...
    return {
      ...
      arguments: {
        ...a,
        title: hit.title,
        // The model never carries the birth YEAR - there is no argument for it -
        // so the sentence's own year is the only source there is.
        ...(hit.dtstart && !a.dtstart ? { dtstart: hit.dtstart } : {}),
        _dated_fact: true,
      },
    };
  });
  if (list.some((tc) => tc && tc.name === "create_reminder_candidate")) return list;
  return [...list, ...facts.map((args) => ({
    name: "create_reminder_candidate",
    arguments: { ...args, _dated_fact: true },
    confidence: 0.95,
  }))];
}

/**
 * Is this the same calendar fact as one already stored? Title is compared
 * case-insensitively and the DATE is compared exactly, because "My birthday" on
 * 19 October and "My birthday" on 14 August are two different claims and only
 * one of them can be right.
 */
function sameCalendarFact(a, b) {
  if (!a || !b) return false;
  const norm = (s) => String(s || "").trim().toLowerCase();
  if (norm(a.title) !== norm(b.title)) return false;
  if (String(a.freq || "").toLowerCase() !== String(b.freq || "").toLowerCase()) return false;
  const num = (v) => (v == null || v === "" ? null : Number(v));
  return num(a.day_of_month) === num(b.day_of_month)
    && num(a.month_of_year) === num(b.month_of_year)
    && String(a.on_date || "") === String(b.on_date || "");
}

/**
 * How many years old this occurrence makes it - "turns 24" - or null.
 *
 * Only computable when the rule carries a real START YEAR, which is the whole
 * reason datedFactReminders keeps one. Returns null rather than 0 for a rule
 * with no year: an unknown age and a newborn must never render the same.
 */
function ageOn(rule, dateKey) {
  if (!rule) return null;
  const start = parseKey(rule.dtstart);
  const on = parseKey(dateKey);
  if (!start || !on) return null;
  if (String(rule.freq || "").toLowerCase() !== "yearly") return null;
  const years = on.y - start.y;
  if (years <= 0 || years > 200) return null;
  return years;
}

/**
 * The window an agenda should actually draw.
 *
 * A calendar whose content is birthdays and quarterly filings is EMPTY in any
 * fixed 21-day window almost all of the time - the owner's live data on
 * 2026-08-08 was two rules, 63 and 72 days out, so agenda, week and month were
 * all blank and the app read as broken. A sentence naming the next date is not
 * a calendar; the fix is to keep widening until there is something to draw.
 *
 * Returns { to, widened } - `widened` so the UI can SAY it looked further,
 * rather than quietly implying the next 300 days are the next 21.
 */
function agendaWindow(rows, today, { days = 21, maxDays = 400 } = {}) {
  const near = addDays(today, days);
  if (!parseKey(today) || !near) return { to: near, widened: false };
  const byDay = agendaBetween(rows || [], today, near);
  if (Object.keys(byDay).length) return { to: near, widened: false };
  const next = upcoming(rows || [], today, { limit: 1, withinDays: maxDays })[0];
  if (!next) return { to: near, widened: false };
  // Reach a fortnight past the first real thing, so a calendar that has one
  // birthday does not open on a list of exactly one row.
  const to = addDays(next.next_due_on, 14);
  return { to: to && compareKeys(to, near) > 0 ? to : near, widened: true };
}
