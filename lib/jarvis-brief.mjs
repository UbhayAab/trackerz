...
function jbCloseDay(input) {
  ...
  return {
    ...
    // HOW MANY MONEY ROWS THERE WERE, so "Rs 0" can be told apart from "nothing
    // logged". `spend` sums the rows that count as spending, and it is 0 in two
    // completely different situations: a day he tracked and genuinely spent
    // nothing, and a day he never opened the app. The closeout was narrating
    // both as "Rs 0 spent" - the same failure the sleepH comment above describes,
    // in the one sentence he actually reads. Counting ROWS, not the sum, is the
    // discriminator: a day of pure income or transfers has money rows and a real
    // Rs 0 of spending, and must still read as a measurement.
    moneyRows: ledger.length,
...
function jbBriefFacts(o) {
  ...
  return {
    ...
    yesterday: y ? {
      ...
      // null, not 0, when the stored day predates the field - see jbSpendPhrase.
      money_rows: y.moneyRows == null ? null : y.moneyRows,
    } : null,
    streaks: o.streaks || {},
    money: jbSafeToSpend({
...
function jbMorningFallback(facts) {
  ...
  if (y && y.logged_anything) {
    var ybits = [jbSpendPhrase(y.spend, y.money_rows == null ? undefined : y.money_rows)];
...
// Money, said honestly. `moneyRows === 0` means no ledger row was written that
// day at all, so there is no measurement to report and "Rs 0 spent" would be an
// assertion the app cannot support. `moneyRows` is undefined on habit_days rows
// stored before this field existed - those are genuinely unknown, so they keep
// the old wording rather than being relabelled either way.
function jbSpendPhrase(spend, moneyRows) {
  if (moneyRows === 0) return "no spending logged";
  return jbRupees(spend) + " spent";
}

function jbCloseoutBody(day, streaks) {
  var bits = [];
  // The cap verdict is only meaningful against something measured: with no money
  // rows there is nothing to be under or over.
  var noMoney = day.moneyRows === 0;
  bits.push(jbSpendPhrase(day.spend, day.moneyRows)
    + (day.caps.spendCap != null && !noMoney ? (day.flags.under_budget ? " (under cap)" : " (over cap)") : ""));
...
export {
  ...
  jbMorningFallback, jbEveningBody, jbMiddayBody, jbCloseoutBody, jbWeeklySummary,
