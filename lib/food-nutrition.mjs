...
export const FOOD_TABLE = [
  ...
  // Tomato rice is the WORST kind of gap in this table, because it did not look
  // like one: "tomato rice" matched "tomato" + "rice" as two separate foods, so
  // `recognized` came back TRUE and 228 kcal became AUTHORITATIVE and overrode
  // the model. A dish that is rice fried in oil with a tempering was priced as a
  // katori of plain boiled rice with a raw tomato next to it - roughly 80 kcal
  // and every gram of the fat missing, silently, with no unknown token to show
  // for it. A wrong number that reports full confidence is worse than one that
  // asks for help, so the composite needs its own row.
  { key: "tomato rice", category: "staple", kind: "count", unit: "1 katori", gramsPerUnit: 150, aliases: ["tomato rice", "thakkali sadam", "tomato bhaat", "tomato pulao"], calories: 300, protein_g: 5.5, carbs_g: 47, fat_g: 10 },
  ...
  // --- the roll shop (Kolkata kathi rolls) ------------------------------------
  // A roll is a fried maida PARATHA wrapped round a filling, and the table knew
  // neither the wrap nor the word. Measured on the owner's own three days:
  // "2 paneer rolls and 1 double egg roll" parsed as 2 x 100 g of loose paneer
  // plus one bare egg - 602 kcal, 42.3 g protein - for a meal that is really
  // ~1330 kcal and ~52 g. Two deep-fried wraps had simply vanished, and the
  // paneer inside a roll is about 60 g, not 200. "aloo roll" matched nothing at
  // all and priced at zero.
  //
  // The egg rolls are three entries rather than one entry times a count, because
  // at a roll shop "double" and "triple" count the EGGS, not the rolls: "one
  // double egg roll" is ONE wrap with two eggs in it. Reading it as two rolls
  // would duplicate the wrap too, and the wrap is the larger half of the meal.
  //
  // There is deliberately no bare "roll" alias. A roll with nothing said about
  // its filling is genuinely ambiguous - a bread roll is a different food - and
  // an ambiguous token belongs with the model. Priced here it would not merely be
  // a guess, it would be a guess that OVERRIDES the model's.
  { key: "egg roll", category: "fastfood", kind: "count", unit: "1 roll", gramsPerUnit: 180, aliases: ["egg roll", "egg rolls", "single egg roll", "anda roll", "egg kathi roll"], calories: 360, protein_g: 12, carbs_g: 36, fat_g: 18 },
  { key: "double egg roll", category: "fastfood", kind: "count", unit: "1 roll (2 egg)", gramsPerUnit: 230, aliases: ["double egg roll", "double egg rolls", "double anda roll"], calories: 430, protein_g: 18, carbs_g: 37, fat_g: 23 },
  { key: "triple egg roll", category: "fastfood", kind: "count", unit: "1 roll (3 egg)", gramsPerUnit: 280, aliases: ["triple egg roll", "triple egg rolls"], calories: 500, protein_g: 24, carbs_g: 37, fat_g: 28 },
  // "pani roll" is not a dish; it is how "paneer roll" comes back from voice
  // capture, which is how this owner logs. Without the alias the word "pani" is
  // already a STOPWORD (water), so the phrase degraded to a bare unknown "roll"
  // and the paneer disappeared without leaving an unknown token behind.
  { key: "paneer roll", category: "fastfood", kind: "count", unit: "1 roll", gramsPerUnit: 220, aliases: ["paneer roll", "paneer rolls", "pani roll", "pani rolls", "panner roll", "panner rolls", "paneer kathi roll"], calories: 450, protein_g: 17, carbs_g: 40, fat_g: 25 },
  { key: "aloo roll", category: "fastfood", kind: "count", unit: "1 roll", gramsPerUnit: 240, aliases: ["aloo roll", "aloo rolls", "potato roll", "potato rolls", "aloo kathi roll"], calories: 430, protein_g: 9, carbs_g: 57, fat_g: 19 },
  { key: "chicken roll", category: "fastfood", kind: "count", unit: "1 roll", gramsPerUnit: 220, aliases: ["chicken roll", "chicken rolls", "chicken kathi roll"], calories: 460, protein_g: 24, carbs_g: 38, fat_g: 23 },
  { key: "veg roll", category: "fastfood", kind: "count", unit: "1 roll", gramsPerUnit: 210, aliases: ["veg roll", "veg rolls", "vegetable roll", "veg kathi roll"], calories: 380, protein_g: 8, carbs_g: 48, fat_g: 17 },
  ...
  // --- quick-service chain items ---
  // Added 2026-08-14, from a real capture ("veg whopper, paneer wrap, med
  // fries") that the table answered with 615 kcal / 28 g protein - about half
  // the truth - because it knew NONE of the three. What it did instead is the
  // part worth remembering: `burger` matched the brand name "Burger King", and
  // `paneer` matched the wrap and priced it as 100 g of RAW PANEER (265 kcal,
  // 18 g protein). Both are confident wrong answers, not gaps, and the table
  // OVERRIDES the model - so nothing downstream could correct them.
  //
  // Numbers are Burger King India's own per-serving figures (fatsecret.co.in,
  // the India catalogue). A chain item is one of the few foods where an exact
  // number genuinely exists, so it is worth carrying rather than estimating.
  { key: "veg whopper", category: "fastfood", kind: "count", unit: "1 burger", gramsPerUnit: 285, aliases: ["veg whopper", "vegetarian whopper", "veggie whopper", "crispy veg whopper", "crunchy veg whopper"], calories: 682, protein_g: 16, carbs_g: 97, fat_g: 25.4 },
  // A wrap is NOT its filling. Sized off BK India's own wraps (the Fiery Chicken
  // Wrap at 252 g / 484 kcal is the best-formed data point in the class); the
  // two catalogue entries for a paneer wrap disagree with each other (406 and
  // 676 kcal at ~230 g), so this sits between them rather than pretending to a
  // precision the source does not have. Paneer over chicken = fattier, less
  // protein.
  //
  // "paneer roll" and "paneer kathi roll" USED to be aliases here and are not any
  // more: the roll shop above claims them, and two entries claiming one alias is
  // decided silently by table ORDER, so reordering the file would have moved a
  // meal's macros without touching a number. A BK wrap and a street kathi roll
  // are not the same product; each keeps its own names.
  { key: "paneer wrap", category: "fastfood", kind: "count", unit: "1 wrap", gramsPerUnit: 230, aliases: ["paneer wrap", "bk paneer wrap"], calories: 500, protein_g: 17, carbs_g: 52, fat_g: 25 },
  { key: "wrap", category: "fastfood", kind: "count", unit: "1 wrap", gramsPerUnit: 240, aliases: ["wrap", "wraps", "kathi roll", "veg wrap", "chicken wrap", "frankie"], calories: 460, protein_g: 16, carbs_g: 51, fat_g: 21 },
  // Fries are sold by SIZE and the sizes are not close: BK India runs 204 / 333
  // / 455 kcal across regular, medium and king. Pricing "med fries" off a single
  // averaged row would be wrong by a meal's worth of a snack in either
  // direction, so each size is its own entry and the longest alias wins. Bare
  // "fries" defaults to medium, which is what a counter hands you when you do
  // not say.
  { key: "small fries", category: "fastfood", kind: "count", unit: "1 regular (72 g)", gramsPerUnit: 72, aliases: ["small fries", "regular fries", "small french fries", "sm fries"], calories: 204, protein_g: 3.6, carbs_g: 27.4, fat_g: 8.9 },
  { key: "fries", category: "fastfood", kind: "count", unit: "1 medium (114 g)", gramsPerUnit: 114, aliases: ["medium fries", "med fries", "french fries", "fries", "finger chips", "medium french fries"], calories: 333, protein_g: 5.9, carbs_g: 43.3, fat_g: 14.4 },
  { key: "large fries", category: "fastfood", kind: "count", unit: "1 large (156 g)", gramsPerUnit: 156, aliases: ["large fries", "king fries", "large french fries", "lg fries"], calories: 455, protein_g: 8, carbs_g: 59.3, fat_g: 19.7 },
...
const STOPWORDS = new Set([
  ...
  // Size abbreviations. Written out they are already stopwords above; typed
  // short they were unknown FOODS, which alone drops recognized to false.
  "med", "reg", "lg", "sm",
]);

// BRAND NAMES THAT CONTAIN A FOOD WORD.
//
// A brand is not a meal. Burger King is the sharp case: the phrase contains the
// alias `burger`, so naming the RESTAURANT logged a 350 kcal burger nobody said
// they ate - and because a matched alias is a confident answer rather than a
// gap, nothing downstream ever flagged it. Masked here, before alias matching,
// so the brand cannot be read as its own menu.
//
// Deliberately short, and it must stay that way. Most food brands SHOULD keep
// matching: someone who says Pizza Hut almost certainly ate pizza, and masking
// that would throw away a true reading to prevent a false one. A name earns a
// place on this list only when the food word in it is the shop's name rather
// than a description of what was eaten.
// Exported so scripts/sync-mirror.mjs can regenerate the edge's literal from
// this runtime value rather than anyone hand-copying it - the same treatment
// FOOD_WORDS and FOOD_TABLE get, and for the same reason.
export const BRAND_PHRASES = ["burger king", "burger singh"];
...
function parsePhrase(phrase, lineQty = null) {
  let masked = ` ${phrase} `;
  // Blank out brand names FIRST, so a restaurant cannot be eaten. Masking keeps
  // the string's length, which every index below depends on, and the leftover
  // sweep turns \0 back into spaces - so a masked brand is neither a food nor an
  // unknown food.
  for (const brand of BRAND_PHRASES) {
    const rx = new RegExp(`(?<![a-z])${brand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![a-z])`, "g");
    masked = masked.replace(rx, (m) => "\0".repeat(m.length));
  }
  const found = [];
