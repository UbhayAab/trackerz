...
// What the row SAYS it will do, in words. The fallback used to be the raw tool
// name, so the feed showed a card reading "remember_fact / pending review" - the
// internal identifier of a function, offered to a human as a decision. There is
// no way to answer that: it does not say what would be remembered, or about what.
// Every tool that can reach this queue now names its own subject.
function reviewLabel(a) {
  const args = a?.arguments || {};
  const tool = a?.tool_name;
  if (tool === "request_user_review") {
    return String(args.raw_text || args.raw_input || args.reason || "Capture needs a look").slice(0, 60);
  }
  if (tool === "remember_fact") {
    const key = String(args.key || "").replace(/[_-]+/g, " ").trim();
    const val = String(args.value ?? "").trim();
    return (key && val ? `Remember: ${key} is ${val}` : `Remember: ${val || key || "a fact"}`).slice(0, 60);
  }
  if (tool === "create_reminder_candidate") return `Remind: ${String(args.title || "a date").trim()}`.slice(0, 60);
  if (tool === "schedule_task_candidate") return `Check later: ${String(args.prompt || "").trim()}`.slice(0, 60);
  if (tool === "set_target_candidate") {
    return `Target: ${String(args.kind || "").replace(/_/g, " ")} ${args.amount ?? ""}`.trim().slice(0, 60);
  }
  if (tool === "update_plan_candidate") {
    return `Plan change: ${String(args.summary || args.kind || "your plan").trim()}`.slice(0, 60);
  }
  if (tool === "amend_log_candidate") return `Change: ${String(args.target_ref || "an earlier entry").trim()}`.slice(0, 60);
  if (tool === "delete_log_candidate") return `Delete: ${String(args.target_ref || "an earlier entry").trim()}`.slice(0, 60);
  return String(args.description || args.merchant || args.note || args.body || "Pending capture").slice(0, 60);
}
...
export function buildAdditions(ledger = [], foods = [], userPlans = [], { limit = 80, notes = [], targetEvents = [], reviewActions = [] } = {}) {
  ...
  for (const a of reviewActions) {
    items.push({
      ...
      label: reviewLabel(a),
      // Can this pending action actually become a row? request_user_review is a
      // question, not a write, so offering "Add it" on one would mark it applied
      // while writing nothing. Everything else names a real write tool.
      applicable: Boolean(a?.tool_name) && a.tool_name !== "request_user_review",
