...
import { stripNegatedClauses } from "./negation.mjs";
...
export function expandToolCalls(toolCalls = [], { evidence = "", now = "" } = {}) {
  ...
  // The POSITIVE half of the capture — clauses asserting something did NOT happen
  // ("didn't go to the gym", "skipped lunch") are stripped, so a skip never salvages
  // a log. `positive === ev` for any capture with no negation (zero behaviour change).
  ...
  if (amount != null && !hasExpense() && !command) {
    const merchant = spendTargetFrom(positive);
    out.push({
      ...
      arguments: {
        ...
        description: positive.replace(MONEY_TRAIL, "").trim().slice(0, 120),
        occurred_at: occurredAt,
  ...
  // 3d. NEGATION OVERRIDE: when a domain's only cue lived in a negated/skipped
  //     clause ("didn't go to the gym", "skipped lunch"), the user is saying it did
  //     NOT happen — drop any log for that domain, even one the MODEL emitted despite
  //     the negation. A mixed "skipped gym but had dal" keeps the positive half,
  //     because `positive` still matches what DID happen. This is the deterministic
  //     layer overruling an untrusted model, and it is what stops a "no-gym" note
  //     from auto-ticking the day's workout.
