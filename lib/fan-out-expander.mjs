...
import { clauseIsAdministrative, isEventDenied, declaresNoWorkout, splitNegationClauses, clauseDeniesEvent, isAlreadyRecorded, alreadyRecordedSpans } from "./negation.mjs";
...
// to see later. As I don't believe the app's ai engine will understand shit."
// produced a correct note AND a correct remember_fact - and then salvage
// fabricated a 432 kcal meal out of it, with the note text itself as the meal
// description. Talking about eating is not eating.
//
// Deliberately narrow, matching only vocabulary a real food/spend log would never
// contain: an explicit note-to-self marker, the app's own internals, or a feature
// request. Habitual and hypothetical phrasing ("I eat eggs daily") is NOT listed -
// too close to a genuine log to risk.
const META_MARKERS = [
  /\bthis is (just )?a note\b/i,
  /\bnote (for|to) (claude|myself|self|later)\b/i,
  /\bfor claude\b/i,
  /\bwhy not\b/i,
  /\bwhy (don'?t|doesn'?t|can'?t|cant|didn'?t)\b/i,
  /\b(json|schema|api|backend|database|codebase|prompt)\b/i,
  /\b(the )?(app|ai) ?(engine|system)\b/i,
  /\bthe app('s)?\b/i,
  /\b(log again|quick add|review queue) (section|tab|screen)?\b/i,
  /\b(feature|bug|ui|screen|button|tab) (request|idea|is broken|should)\b/i,
];

export function isAboutTheApp(text = "") {
...
// A DATE THAT BELONGS TO A FACT IS NOT THE DATE OF THE EVENT.
//
// 2026-08-06, live: "My bday is 19 oct 2002\n\nAlso, I had 50g aloo bhujia and 2
// soop sachet, the same as yesterday" was salvaged into a food_log whose
// occurred_at was 2026-10-19 - seventy-four days in the FUTURE. The real lunch
// vanished from that day's totals and a phantom one is sitting in October. The
// month-name branch below read "19 oct" out of the BIRTHDAY and, because that
// branch never reads a year, dropped the "2002" that would have made the misread
// obvious. The birth date belongs to the birthday; the meal happened now unless
// the sentence says otherwise. So the fact clause is blanked before any date is
// read out of the text. Bounded by sentence punctuation and a newline, so a date
// in the NEXT clause ("...and I ate biryani on 25 Jun") is untouched.
...
export function resolveOccurredAt(text = "", now = "") {
  ...
  // A SALVAGED LOG IS A RECORD OF SOMETHING THAT ALREADY HAPPENED. Every target
  // salvage can write - food, workout, expense, hydration, body metric - is a
  // past event, so a resolved date in the FUTURE is by construction a misread and
  // never an intention. This is the clock-only backstop behind the fact-clause
  // mask above: that one has to recognise a phrasing, this one needs nothing but
  // a calendar, and on its own it would have stopped the October lunch.
  //
  // A bare month-day rolls back a YEAR rather than collapsing to today, because
  // "September 14" said in June names last September and the day the user
  // actually stated is worth keeping. That is the same rule lib/amend-target.mjs
  // already applies to the same ambiguity. A date that STATED a future year said
  // something that cannot be a log at all, so it falls back to the capture
  // moment. The wall time is read as IST, so the instant is `at` minus the offset.
  const isFuture = (dt) => dt.getTime() - 5.5 * 3_600_000 > base.getTime() + 60_000;
  if (isFuture(at)) {
...
// may not produce any of these - see the veto in expandToolCalls.
...
// delta is the opposite: the SYSTEM_PROMPT's LOG-THAT-CONTRADICTS-THE-PLAN rule
// deliberately pairs one with a real log ("planned leg day, I did cardio"), and
// a bare denial ("...and no gym today") also draws a same-day rest delta out of
// the model while the same breath logs 500 g of curd. Treating those as commands
// silently ate the food.
//
...
export function expandToolCalls(toolCalls = [], { evidence = "", now = "" } = {}) {
  ...
  // A command OR meta-commentary about the app. Both mean "this is not a log", so
  // every deterministic salvage below stays off; the model's own note /
  ...
  // (gymDenied / foodDenied are computed above, because `command` depends on them:
  // the capture names a domain only to say it did NOT happen - "no gym today",
  // "skipped lunch". Salvage stays off for that domain and any positive row the
  // model emitted is dropped, because a denial is evidence AGAINST the event and
  // outranks a low-confidence guess.)
  ...
  // `skipped` row - which here is a claim that the owner answered 20 September
  // and did not train. They said nothing of the sort; they wrote a reminder.
  // A skipped row is still a row, and it still landed in the review queue that
  // this whole session is about, so the answer is to emit nothing at all.
  ...
  //     log with its date-scoped delta.
