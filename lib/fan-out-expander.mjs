...
import { isEventDenied, declaresNoWorkout } from "./negation.mjs";
 ...
 *
 * "Restatement" is judged against the EVIDENCE, not against the row: the note
 * and the row are both derived from the same sentence, so if the note adds no
 * word the sentence did not already have, it adds nothing at all. A note that
 * introduces its own content survives.
 */
export function dropRestatingNotes(toolCalls = [], evidence = "") {
  ...
  return calls.filter((tc) => {
    ...
    // Words the note has that the capture did not - genuinely new content.
    let novel = 0;
    for (const w of noteBag) if (!evBag.has(w)) novel += 1;
    return novel / noteBag.size > 0.5;
  });
}
