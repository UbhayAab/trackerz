...
export const GYM_WORDS = [
  // Bare "workout"/"work out"/"exercise" count as a gym mention. They were absent
  // here while the edge function's GYM_CUE mirror had them, so the two halves of
  // the pipeline disagreed about what a workout capture even is - the edge salvaged
  // rows this module said were not gym text. Negation (lib/negation.mjs) is what
  // keeps the broader cue safe: mentioning a workout is not claiming to have done one.
  "workout", "work out", "exercise", "exercised",
